...
`getString()` triggers one reading of both chips through the [NW-Device-Specification](https://github.com/NorthernWidget/NW-Device-Specification) handshake (`updateMeasurements()`), waits for the device's reading counter, and prints pressure [hPa = mBar], relative humidity [%], and the LPS35HW and SHT31 temperatures [°C], with `-9999.00` where a reading failed. The getters return the stored reading (`getPressure()`, `getHumidity()`, `getTemperature(RH_Sense)` or `getTemperature(Pres_Sense)`); pass `true` to take a fresh one first. `setHumidityReadings(n)` and `setPressureReadings(n)` set how many readings of the SHT31 (humidity and its temperature) and of the LPS35HW (pressure and its temperature) each `getString()` takes (clamped to `HAAR_HUMIDITY_CAPACITY` and `HAAR_PRESSURE_CAPACITY`, default 16; override before the include); the values printed are then the means, and `getHumidityMean()`, `getHumidityStd()`, `getHumiditySterr()`, `getHumidityMedian()`, the same for pressure and for `getTemperature…(RH_Sense)` or `(Pres_Sense)`, plus `getHumidityCount()` and `getPressureCount()`, read the stored readings. With `setHumidityStats(true)` or `setPressureStats(true)` the std and sterr columns join `getHeader()` and `getString()`. `updateMeasurements(Haar::SHT31)` or `(Haar::LPS35HW)` reads one chip group alone; `updateMeasurements(false)` only requests a reading, which `newData()` captures. For one row per reading to a file, `beginReadings(component, n)`, `printHeader(out)`, `logReading(out)` n times, `endReadings()`, to any `Print` (an SdFat `File`, `Serial`).

Faults, for sketches that want them: `faulted(chip)`, `anyFault()`, `faultChip()`, `faultKind()`, `printFault(Serial)`, `faultNote()` (one word, e.g. `SHT31Checksum`, for a logger's note column). `begin()` refuses a device that is not Schema 1, not a Haar, or below firmware patch `HAAR_FW_MIN_PATCH`; `beginFailure()` says which. The default address is `0x48` (Schema 1); firmware before Schema 1 answered at `0x42` and is not supported by this version. Requires the [NW_Core](https://github.com/NorthernWidget/NW_Core) library.
